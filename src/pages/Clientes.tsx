...
export const Clientes = () => {
  const { clientes, adicionarCliente, atualizarCliente, removerCliente } = useArmazem();
  ...
  const handleSalvar = () => {
    ...
    if (tipoDoc === 'CPF' && numeroDoc.length !== 11) {
      alert("Erro de Validação: O CPF brasileiro deve conter exatamente 11 dígitos numéricos.");
      return;
    }
    if (tipoDoc === 'RG' && (numeroDoc.length < 7 || numeroDoc.length > 9)) {
      alert("Erro de Validação: O RG deve conter entre 7 e 9 dígitos numéricos.");
      return;
    }
    if (tipoDoc === 'Passaporte' && numeroDoc.length !== 8) {
      alert("Erro de Validação: O Passaporte deve conter exatamente 8 caracteres alfanuméricos.");
      return;
    }
    ...
    if (modoModal === 'criar') {
      ...
      if (documentoJaExiste) {
        alert(`Integridade violada: Este número de ${tipoDoc} já está cadastrado no sistema.`);
        return;
      }
    }

    const payloadCliente: Cliente = {
      id: modoModal === 'editar' && idSelecionado ? idSelecionado : crypto.randomUUID(),
    ...
    if (modoModal === 'editar') {
      atualizarCliente(payloadCliente);
    } else {
      adicionarCliente(payloadCliente);
    }
              ...
              clientes.map((cliente) => (
                <TableRow key={cliente.id} hover sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                  ...
                  <TableCell align="right">
                    ...
                    <Tooltip title="Excluir"><IconButton color="error" onClick={() => { if(window.confirm(`Deseja excluir ${cliente.nome}?`)) removerCliente(cliente.id); }}><DeleteIcon /></IconButton></Tooltip>
                  </TableCell>
                </TableRow>
              ))
            )}
